...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BRIGMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BRIGMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BRIGMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("brig-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("BRIGMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running BRIGMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadBRIGMiner() exception");
    } catch (...) {
        LogPrintf("ThreadBRIGMiner() exception");
    }

    LogPrintf("ThreadBRIGMiner exiting\n");
}
